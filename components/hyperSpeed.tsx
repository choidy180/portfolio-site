...
/** --- Distortion uniforms (typed) --- */
const mountainUniforms: Record<string, Uniform> = {
  uFreq: { value: new THREE.Vector3(3, 6, 10) },
  uAmp: { value: new THREE.Vector3(30, 30, 20) },
};

const xyUniforms: Record<string, Uniform> = {
  uFreq: { value: new THREE.Vector2(5, 2) },
  uAmp: { value: new THREE.Vector2(25, 15) },
};

const LongRaceUniforms: Record<string, Uniform> = {
  uFreq: { value: new THREE.Vector2(2, 3) },
  uAmp: { value: new THREE.Vector2(35, 10) },
};

const turbulentUniforms: Record<string, Uniform> = {
  uFreq: { value: new THREE.Vector4(4, 8, 8, 1) },
  uAmp: { value: new THREE.Vector4(25, 5, 10, 10) },
};

const deepUniforms: Record<string, Uniform> = {
...
const distortion_uniforms: Record<string, Uniform> = {
...
/** NEW: BufferGeometry → InstancedBufferGeometry 복사 헬퍼 */
function toInstanced(geom: THREE.BufferGeometry): THREE.InstancedBufferGeometry {
  const inst = new THREE.InstancedBufferGeometry();

  const index = geom.getIndex();
  if (index) inst.setIndex(index.clone());

  for (const name in geom.attributes) {
    const attr = geom.attributes[name] as THREE.BufferAttribute | THREE.InterleavedBufferAttribute;
    if (attr instanceof THREE.BufferAttribute) {
      // BufferAttribute는 clone 가능
      inst.setAttribute(name, attr.clone());
    } else {
      // InterleavedBufferAttribute는 clone이 없으니 참조를 그대로 사용
      inst.setAttribute(name, attr);
    }
  }

  if (geom.morphAttributes) {
    inst.morphAttributes = { ...geom.morphAttributes };
  }

  if (geom.boundingBox) inst.boundingBox = geom.boundingBox.clone();
  if (geom.boundingSphere) inst.boundingSphere = geom.boundingSphere.clone();

  inst.drawRange.start = geom.drawRange.start;
  inst.drawRange.count = geom.drawRange.count;
...
class CarLights {
  ...
  init() {
    ...
    const instanced = toInstanced(geometry);
  ...
  update(time: number) {
    const u = this.mesh.material.uniforms as Record<string, Uniform>;
...
interface Assets {
  smaa: { search?: HTMLImageElement; area?: HTMLImageElement };
}
...
class App {
  ...
  fogUniforms: Record<string, Uniform>;
  fovTarget: number;
