...
/** onBeforeCompile 파라미터 타입을 Three에서 추론해서 사용 */
type OnBeforeCompileShader = Parameters<MeshPhysicalMaterial['onBeforeCompile']>[0];
...
class X {
  #config: XConfig;
  #postprocessing: PostProcessingLike | null = null;
  ...
  #initObservers() {
    ...
    const onVisibility = this.#onVisibilityChange.bind(this);
  ...
  get postprocessing(): PostProcessingLike | null {
    return this.#postprocessing;
  }
  set postprocessing(value: PostProcessingLike | null) {
  ...
  dispose() {
    ...
    (this.renderer.getContext() as any)?.getExtension?.('WEBGL_lose_context')?.loseContext?.();
...
class Y extends MeshPhysicalMaterial {
  ...
  constructor(params: Partial<MeshPhysicalMaterialParameters>) {
    super(params);
    this.defines = { USE_UV: '' };
    this.onBeforeCompile = (shader: OnBeforeCompileShader) => {
  ...
  onBeforeCompile2?: (shader: OnBeforeCompileShader) => void;
}

const BALLPIT_DEFAULTS = {
  count: 200,
  colors: [0xff3cac, 0x784ba0, 0x2b86c5],
...
type BallpitConfig = typeof BALLPIT_DEFAULTS;
...
class Z extends InstancedMesh {
  ...
  constructor(renderer: WebGLRenderer, params: Partial<BallpitConfig> = {}) {
    const config: BallpitConfig = { ...BALLPIT_DEFAULTS, ...params };
